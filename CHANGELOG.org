...
Eliminated the white flash in =my/consult-theme= on open, on each preview step, on =C-g= abort, and on commit. Root cause was =load-theme= being called when it didn't need to be — re-evaluating an already-active theme on open, and reloading the saved themes to restore them on =C-g= — each call firing =enable-theme-functions= (and =selected-window-accent-mode='s theme reaction) and producing a visible redraw. The preview now (a) skips =load-theme= for already-enabled themes, promoting them with =enable-theme= instead; (b) stacks preview themes on top of the saved themes instead of swapping, so the saved themes are never disabled and =C-g= can restore them by =disable-theme=-ing alone (no reload); and (c) wraps every load/enable/disable in =inhibit-redisplay= so theme-change hooks can't draw an intermediate frame. The commit body likewise promotes with =enable-theme= when the chosen theme is already loaded.

** <2026-05-08>

Added =my/diff-minimap-toggle= — a side-window minimap that renders diff-hl hunk status as coloured rows showing added, changed, and removed regions alongside a viewport indicator and cursor marker. Togglable via =C-x v m=, with mouse-click navigation and automatic sync on scroll and save.

** <2026-04-26>

Added =persistent-scratch= replacement — saves the =*scratch*= buffer to =~/.emacs.d/.scratch= on every edit via a buffer-local =after-change-functions= hook, so even a hard crash loses at most the last keystroke. Restores from that file on startup.

Replaced =toggle-centered-buffer= with =my/center-buffer-mode=, a proper minor mode that recalculates margins on window resize, has a modeline lighter, and cleanly toggles =visual-line-mode= on/off.

Disabled =diff-hl-dired-mode= hook to avoid conflict with the existing =emacs-solo/dired-git-status-overlay=.

** <2026-04-24>

Added an async-transfer header-line indicator and an rsync-based dired copy. The indicator replaces =dired-async='s far-right modeline lighter — easy to miss with splits — with a global =header-line= that only appears while jobs are running, counting both =dired-async= and rsync jobs in the same spot. Critically it never mutates =mode-line-format=. The rsync backend uses only =start-process= so it works on older Emacs without =async.el=, shells out to =rsync -ah --info=progress2=, streams live =[NN% NN.NMB/s]= progress into the header-line, handles moves via =--remove-source-files=, and translates =/ssh:= TRAMP endpoints to rsync-over-ssh.

** <2026-04-17>

Added a =consult-theme= replacement with live preview — previews each candidate as you step through the =completing-read=, restores the originally enabled themes on =C-g=, and pairs with the existing =enable-theme-functions= hook so the accent-sync fires on every preview step. Bound to =M-m=. Also fixed =my/sync-ui-accent-color= so it no longer prompts when called non-interactively (from startup or theme-change hooks) — it now caches the last-used color in =my/sync-ui-accent-color--current= and reuses it as the fallback.

** <2026-04-09>

Added a dired-collapse replacement that squashes single-child directory chains into one dired line, using only built-in Emacs features. Rewrites the filename column in place, reapplies the dired-filename text property, and runs before the icon overlays so icons attach to the collapsed lines.
